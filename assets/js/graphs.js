...
const monthDays = Array.from({length: getDaysInMonth(4,2021)}, (_, i) => i + 1);
...
jQuery(document).ready(function ($) {
    ...
    const dvisitorData = monthDays.map(day => {
        const record = wp_ajax.visit_data.find(data => day == data.visited.split('-')[2]);
        return record ? Number(record.count) : 0;
    });
  ...
  const visitorGraphData = {
    ...
    $("#submitData").click(()=>{
        ...
        $.ajax({
          ...
          const labels = groupBy =='yearly' ? months : graphData.map(d=>d.label);
